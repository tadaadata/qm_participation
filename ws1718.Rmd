...
Wir erhalten eine **`r ifelse(totals$median < totals$mean, "linkssteile", "rechtssteile")`** Verteilung. Der Shapiro-Wilk-Test auf Normalverteilung ergab ein **`r ifelse(shap_p < .10, "signifikantes", "nicht signifikantes")`** Ergebnis `r ifelse(shap_p < .10, paste0("**(p ", pval_string(shap_p), ")**"), "")` (was allein.
...
# I'll leave it by this until I get my actual idea to work
mean_by_tutid %>% 
  # filter(!is.na(upper)) %>% 
  ggplot(aes(x = TutID, color = TutorIn, y = mean)) +
  geom_errorbar(aes(ymin = lower, ymax = upper), 
                size = 2, width = .5, color = "black") +
  geom_point(size = 2.5, shape = 23, stroke = 1.5, color = "black") +
  geom_errorbar(aes(ymin = lower, ymax = upper), 
                size = 1, width = .4) +
  geom_point(size = 1.5, shape = 23, stroke = 1, fill = "white") +
...
mean_by_day %>%
  ggplot(aes(x = Tag)) +
  geom_errorbar(aes(y = mean, ymin = lower, ymax = upper), size = 1, width = .6) +
  geom_point(aes(y = mean), color = "white", fill = "red", shape = 23, size = 2.5, stroke = 1) +
  geom_hline(aes(yintercept = mean(mean)), linetype = "dotted") +
  scale_y_continuous(breaks = seq(0, 100, 5),
                     minor_breaks = seq(0, 100, 1)) +
  expand_limits(y = 0) +
...
participation %>% group_by(Aufgabenblatt) %>%
  ...
  geom_errorbar(aes(y = mean, ymin = lower, ymax = upper), size = .8, width = .7) +
  geom_point(aes(y = mean), color = "white", fill = "red", shape = 23, size = 2.5, stroke = 1) +
  geom_hline(aes(yintercept = mean(mean)), linetype = "dotted") +
  scale_y_continuous(breaks = seq(0, 100, 5),
                     minor_breaks = seq(0, 100, 1)) +
...
```{r verlauf_pertut}
# experiemnt: from (stacked) bar charts to dots & lines
# also: I always seem to forget what to do about 
# "geom_path: Each group consists of only one observation. Do you need to adjust the group aesthetic?"
# Plot seems fine, though
participation %>% 
  mutate(Uhrzeit = factor(Uhrzeit, levels = rev(levels(Uhrzeit)), ordered = TRUE)) %>%
  ggplot(aes(x = Aufgabenblatt, y = Teilnehmerzahl, fill = TutorIn, color = TutorIn, group = TutorIn)) +
  # geom_bar(stat = "identity", color = "black") +
  geom_line(size = .75) +
  geom_point(size = 1.5, shape = 21, stroke = .75, color = "white") +
  scale_fill_brewer(palette = "Set1") +
  scale_color_brewer(palette = "Set1") +
  facet_grid(Uhrzeit ~ Tag, switch = "y") +
  labs(title = "Teilnehmende pro Woche pro Tutorium", 
       x = "Aufgabenblatt", fill = "", color = "", y = "") +
  theme_readthedown(axis_emph = "y") +
